    ...
    <form class="grid grid-cols-2 max-ipad:grid-cols-1 gap-3" action="/customer/add" method="POST">
        ...
        <div class="bg-white rounded-md p-3 shadow-1 h-max">
            ...
            <div class="mt-2">
                ...
                <input type="text" id="phone" name="phone" value="{{ old('phone') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
                @error('phone')
                    <p class="mt-0.5 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
