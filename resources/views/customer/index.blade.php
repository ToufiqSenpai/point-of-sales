...
@section('main')
    <div class="bg-white rounded-md w-full p-3 min-h-full shadow-1">
        <div class="flex justify-end">
            <a href="/customer/add" class="bg-green-500 text-white px-3 py-1 rounded-md font-medium ml-auto">Tambah Customer</a>
        </div>
        <form class="flex items-center mt-3">
            ...
            <div class="relative w-full">
                ...
                <input type="text" id="simple-search" name="search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" placeholder="Search supplier..." required>
            </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-3">
                ...
                @foreach($customers as $customer)
                    <tr class="@if($loop->index % 2 == 0) bg-white @else bg-gray-50 @endif border-b dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 flex gap-1">
                            <a href="/customer/edit/{{ $customer['id'] }}" class="text-sm hover:underline">
                            ...
                            <x-modal.delete description="Apakah anda ingin menghapus {{ $customer['name'] }}" action="/customer" id="{{ $customer['id'] }}" />
                        </td>
                    </tr>
                @endforeach
                </tbody>
            ...
            <nav class="flex items-center justify-between py-4 px-6 w-full" aria-label="Table navigation">
                <span class="text-sm font-normal text-gray-500 dark:text-gray-400">Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $customers->firstItem() }}-{{ $customers->lastItem() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $customers->total() }}</span></span>
