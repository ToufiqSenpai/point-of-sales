...
@section('main')
    <div class="bg-white rounded-md w-full p-3 min-h-full shadow-1">
        <div class="flex justify-end">
            <a href="/product/category/add" class="bg-green-500 text-white px-3 py-1 rounded-md font-medium ml-auto">Tambah Category</a>
        </div>
                ...
                @foreach($categories as $category)
                    <tr class="@if($loop->index % 2 == 0) bg-white @else bg-gray-50 @endif border-b dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 flex gap-1">
                            <a href="/product/category/edit/{{ $category['id'] }}" class="text-sm hover:underline">
                            ...
                            <x-modal.delete description="Apakah anda ingin menghapus {{ $category['name'] }}" action="/product/category" id="{{ $category['id'] }}" />
                        </td>
                    </tr>
                @endforeach
                </tbody>
